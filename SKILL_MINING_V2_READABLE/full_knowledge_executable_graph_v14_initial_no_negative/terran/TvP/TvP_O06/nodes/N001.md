...
Semantically aligned validated knowledge node: `N004` (alignment score 0.189). The executable policy above remains primary; use this overlay only for factual feasibility, candidate selection, and repair checks.

### Human-Trajectory Interpretation

The human response evidence contains broad investment involving Marine, WidowMine, SupplyDepot, Starport, SiegeTank; retain its statistical sign without copying an action sequence.
...
**Human-trajectory candidate pool (not an ordered build list):** Marine, WidowMine, SiegeTank, Marauder
