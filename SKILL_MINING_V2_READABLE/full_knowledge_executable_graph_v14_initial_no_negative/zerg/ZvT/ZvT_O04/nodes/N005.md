# N005 — Late-Midgame Ground and Air

## Node Type

DEFAULT

## Summary

Late-midgame, opponent ground with Siege Tanks, Marines, Medivacs, own ground with Zerglings, Mutalisks, Queens. Maintain ground strength, continue economy, increase tech, consider air.

## When This Applies

### Opponent cues

- Enemy Intelligence shows ground posture with Siege Tanks, Marines, Medivacs, heavy production and tech.
- Treat remembered or observed Enemy Intelligence as partial and uncertain.

### Own cues

- Late-midgame, heavy economy, heavy production, heavy tech, ground and air mix.
- Use the live observation to check army supply, free supply, resource bank, technology, and current queues.
- These cues are approximate and do not all need to be true.

## Recommended Strategic Direction

Maintain ground strength, continue economy, increase tech, consider air.

Diversify army to handle Terran ground compositions.

## What This Does NOT Mean

This node is not an instruction to reproduce a historical action sequence.

Choose exact macro actions from the current live observation, not merely because a unit or structure appeared in historical evidence.

## Transition Goal

Build a balanced army for late game.

## Knowledge Constraint Overlay

Semantically aligned validated knowledge node: `N005` (alignment score 0.162). The executable policy above remains primary; use this overlay only for factual feasibility, candidate selection, and repair checks.

### Human-Trajectory Interpretation

Human trajectories show a heavy economy/upgrade/expansion posture with ground army focus. The presence of Zergling, Mutalisk, Queen, and structures like EvolutionChamber and Lair indicates a balanced tech and production approach.

### Applicability Checks

- Check if economy is still heavy and expansions are safe.
- Check if opponent's ground army is still the primary threat.
- Check if upgrades and tech are progressing as planned.
- Check if creep spread is adequate for defense and map control.

### Stop / Repair / Recheck

If resources are not converting into army strength, recheck production and tech priorities; if repeated failed posture, adjust strategy based on live observations.

### Knowledge-Grounded Execution Envelope

**Human-trajectory candidate pool (not an ordered build list):** Zergling, Mutalisk, Queen, Overlord, Roach

- Selection: Choose only a currently reachable candidate after checking live producers, prerequisites, resources, supply, and active queues; this is a candidate pool, not an ordered build list.
- Resource conversion: When the combined bank is at least 750 and army supply is still below 15 after 05:00, prefer a currently executable combat candidate from this pool before optional greed or deeper technology.
- Unreachable-candidate fallback: If the preferred candidate is not reachable before the next decision, use a cheaper currently producible trajectory candidate; do not queue a long blocked prerequisite chain while army supply is low.
- Feedback repair: If the bank/low-army deficit persists for two decisions, stop repeating the same plan, re-read the best matching node, and choose a different reachable candidate or producer bottleneck repair.

