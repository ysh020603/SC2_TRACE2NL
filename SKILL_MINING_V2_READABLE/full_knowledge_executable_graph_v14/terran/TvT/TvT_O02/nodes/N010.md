...
Semantically aligned validated knowledge node: `N004` (alignment score 0.157). The executable policy above remains primary; use this overlay only for factual feasibility, candidate selection, and repair checks.

### Human-Trajectory Interpretation

The human response evidence contains broad investment involving Marine, Starport, Barracks, Factory, SupplyDepot; retain its statistical sign without copying an action sequence.
