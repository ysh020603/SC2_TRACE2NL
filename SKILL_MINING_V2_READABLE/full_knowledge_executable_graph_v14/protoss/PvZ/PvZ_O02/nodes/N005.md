...
Semantically aligned validated knowledge node: `N010` (alignment score 0.229). The executable policy above remains primary; use this overlay only for factual feasibility, candidate selection, and repair checks.

### Human-Trajectory Interpretation

The human response evidence contains broad investment involving Zealot, Stalker, Sentry, Observer, Pylon; retain its statistical sign without copying an action sequence.
...
**Human-trajectory candidate pool (not an ordered build list):** Zealot, Stalker, Sentry, Observer, Immortal, WarpPrism, DarkTemplar
